...
def akun_registrasi(request):
    # jika user sudah registrasi maka tidak boleh akses fungsi ini lagi
  if request.user.is_authenticated:
      return redirect('/')
  pesan = ""
  template_name = "halaman/akun_registrasi.html"
  if request.method == "POST":
...
def akun_logout(request):
    logout(request)
    return redirect('/')  # Ganti 'login' dengan nama URL untuk halaman login Anda
